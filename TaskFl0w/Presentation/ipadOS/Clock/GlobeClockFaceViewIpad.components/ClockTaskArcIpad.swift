...
struct ClockTaskArcIpad: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack {
                ...
                .stroke(task.category.color, lineWidth: 28)  // Увеличенная ширина линии для iPad
                .gesture(
                    TapGesture()
                        .onEnded {
                            withAnimation {
                ...
                Image(systemName: task.category.iconName)
                    .font(.system(size: 18))  // Увеличенный размер шрифта для iPad
    ...
    private func timeForLocation(_ location: CGPoint, center: CGPoint) -> Date {
        ...
        var components = Calendar.current.dateComponents(
            [.year, .month, .day], from: task.startTime)
